...
class FacetCount(AppwriteModel, Generic[T]):
    """
    Facet values and their counts for one faceted field.

    Attributes
    ----------
    counts : Optional[List[Any]]
        Typed model field.
    field_name : Optional[str]
        Typed model field.
    """
    counts: Optional[List[Any]] = Field(default=None, alias='counts')
    field_name: Optional[str] = Field(default=None, alias='field_name')

    @classmethod
    def with_data(cls, data: Dict[str, Any], model_type: Type[T] = dict) -> 'FacetCount[T]':
        """Create FacetCount instance with typed data."""
