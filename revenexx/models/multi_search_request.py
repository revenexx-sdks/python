...
class MultiSearchRequest(AppwriteModel, Generic[T]):
    """
    ...
    Attributes
    ----------
    searches : List[MultiSearchEntry[T]]
        The searches to run, in order. Must not be empty.
    """
    searches: List[MultiSearchEntry[T]] = Field(..., alias='searches')

    @classmethod
    def with_data(cls, data: Dict[str, Any], model_type: Type[T] = dict) -> 'MultiSearchRequest[T]':
        """Create MultiSearchRequest instance with typed data."""
