...
from .exception import RevenexxAPIRevenexxException
...
class Client:
    def __init__(self):
        ...
        self._global_headers = {
            'content-type': '',
            'user-agent' : f'RevenexxAPIRevenexxPythonSDK/0.0.1 ({platform.uname().system}; {platform.uname().version}; {platform.uname().machine})',
    ...
    def set_endpoint(self, endpoint):
        if not endpoint.startswith('http://') and not endpoint.startswith('https://'):
            raise RevenexxAPIRevenexxException('Invalid endpoint URL: ' + endpoint)
    ...
    def call(self, method, path='', headers=None, params=None, response_type='json'):
        ...
        try:
            ...
            if response != None:
                content_type = response.headers['Content-Type']
                if content_type.startswith('application/json'):
                    raise RevenexxAPIRevenexxException(response.json()['message'], response.status_code, response.json().get('type'), response.text)
                else:
                    raise RevenexxAPIRevenexxException(response.text, response.status_code, None, response.text)
            else:
                raise RevenexxAPIRevenexxException(e)
    ...
    def chunked_upload(
        ...
        while offset < size:
            ...
            if "$id" in result:
                headers["x-revenexx api — revenexx-id"] = result["$id"]
